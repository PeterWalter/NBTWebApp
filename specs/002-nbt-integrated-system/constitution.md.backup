...
```yaml
Base Framework: Blazor Web App Interactive Auto (.NET 8+)
...
New Entities (10 required):
  ...
  8. TestResult (scores, upload tracking)
  9. SpecialSession (remote writer, special accommodations)
  10. AuditLog (comprehensive activity logging)
...
Steps:
  1. Personal Information (ID validation, duplicate check)
  2. NBT Number Generation (Luhn algorithm, unique check)
  3. Test Session Selection (capacity validation)
  4. Payment Information (EasyPay initiation)
  5. Confirmation (summary, submission)

Persistence Strategy: Save draft on each step
Validation: Client + server-side per step
Navigation: Forward, backward, cancel with state preservation
```

**PRINCIPLE:** Registration MUST be resumable. Partial data MUST be persisted to prevent data loss.
